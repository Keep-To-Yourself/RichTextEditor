	...
	public func toggleBlockquote() {
		...
		for paraRange in affectedParagraphRanges.reversed() {  // 从后向前处理以避免范围问题
			...
			let isOriginallyListItem =
				...
				&& ((currentAttributes[NSAttributedString.Key.blockType]
					as? String == "list")  // 原本是纯列表项
					|| (currentAttributes[NSAttributedString.Key.blockType]
						as? String == "blockquote"))  // 或者已经是被引用的列表项

			if convertToBlockquoteStyle {
				// --- 转换为引用块样式 ---
				var newAttrs = currentAttributes  // 基础属性，保留内联样式等
				newAttrs[NSAttributedString.Key.blockType] = "blockquote"
				newAttrs[NSAttributedString.Key.paragraphStyle] =
					BlockquoteContent.getParagraphStyle(level: 0)  // 引用块的基础缩进
				newAttrs[NSAttributedString.Key.blockID] =
					currentAttributes[NSAttributedString.Key.blockID] ?? UUID()  // 保留或生成新的块ID
				...
				if isOriginallyListItem {
					...
					newAttrs.removeValue(
						forKey: NSAttributedString.Key.metadata
					)
				}

				textStorageRef.setAttributes(newAttrs, range: paraRange)
				...
				if isOriginallyListItem {  // 原本是“被引用的列表项”，现在变回“普通列表项”
					...
					if let listLevel = currentMetadata?["level"] as? Int {
						newAttrs[NSAttributedString.Key.paragraphStyle] =
							ListContent.getParagraphStyle(level: listLevel)  // 恢复列表项的缩进
					} else {
						newAttrs.removeValue(
							forKey: NSAttributedString.Key.paragraphStyle
						)  // 元数据不完整，移除特定样式
